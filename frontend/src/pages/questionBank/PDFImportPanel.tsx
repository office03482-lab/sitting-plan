...
import { AlertTriangle, CheckSquare, FileUp, Loader2 } from 'lucide-react';
...
export default function PDFImportPanel({
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(requestError, 'PDF import failed.'));
      setQuestions([]);
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center hover:border-rose-400 hover:bg-rose-50">
        ...
        <span className="text-[11px] font-medium text-slate-600">{fileName || 'Upload PDF document'}</span>
        <input
          type="file"
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-1 text-[10px] font-semibold text-white hover:bg-rose-700"
            >
              ...
              <label className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1 space-y-2">
                  <textarea
                    ...
                    rows={3}
                    className="w-full rounded-md border border-slate-200 bg-white px-2 py-1.5 text-[11px] text-slate-800 outline-none focus:border-rose-400"
                  />
                  <textarea
                    ...
                    className="w-full rounded-md border border-slate-200 bg-white px-2 py-1.5 text-[11px] text-slate-700 outline-none focus:border-rose-400"
                  />
                  <input
