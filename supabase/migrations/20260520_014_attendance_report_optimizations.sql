...
create index if not exists idx_student_attendance_school_student_date_created
on attendance.student_attendance (school_id, student_id, attendance_date desc, created_at desc);

create index if not exists idx_student_attendance_school_date_created
on attendance.student_attendance (school_id, attendance_date desc, created_at desc);

create index if not exists idx_students_school_active_class_section_name
on public.students (school_id, is_active, class_name, section, full_name);

create index if not exists idx_students_school_active_roll_number
...
is 'Optimized attendance report reader with DB-side filtering, joins, and pagination to avoid large PostgREST IN-chunk fanout.';
