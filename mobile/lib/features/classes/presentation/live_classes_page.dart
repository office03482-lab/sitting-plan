...
class LiveClassesPage extends ConsumerWidget {
  const LiveClassesPage({
    this.title = 'Live Classes',
    this.subtitle = 'Join links, recordings, notes, and attendance-aware class history.',
    super.key,
  });
              ...
              child: items.isEmpty
                  ...
                  : Column(
                      children: items.take(20).map<Widget>((Map<String, dynamic> row) {
                        ...
                        final meetingLink = displayValue(row['meeting_link'], fallback: '');
                        final recordingUrl = displayValue(row['recording_url'], fallback: '');
                        final notesUrl = displayValue(row['notes_url'], fallback: '');
                        return ListTile(
                          contentPadding: EdgeInsets.zero,
                          leading: const CircleAvatar(child: Icon(Icons.video_camera_front_outlined)),
                          title: Text(
                            '${displayValue(timetable['class_name'], fallback: 'Class')} - ${displayValue(timetable['subject'], fallback: displayValue(row['provider']))}',
                          ),
                          subtitle: Column(
                            ...
                            children: <Widget>[
                              ...
                              if (meetingLink.isNotEmpty) Text('Join: $meetingLink'),
                              if (recordingUrl.isNotEmpty) Text('Recording: $recordingUrl'),
                              if (notesUrl.isNotEmpty) Text('Notes: $notesUrl'),
                            ],
                          ),
